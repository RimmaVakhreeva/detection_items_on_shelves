...
class GetEmbeddings(nn.Module):
    ...
    def __init__(self, num_classes, last_stride, neck,
        ...
        self.neck = neck
        self.neck_feat = neck_feat # количество каналов в последнем слое
    ...
    def get_embeddings(self, images: List[np.ndarray]) -> np.ndarray:
        resized_images = []
        for img in images:
            resized_images.append(cv2.resize(img, (128, 128)))

        crops = np.stack(resized_images, axis=0)
        crops = ((crops / 255.) - self.pixel_mean) / self.pixel_std

        crops = crops.transpose((0, 3, 1, 2)).astype(np.float32)
